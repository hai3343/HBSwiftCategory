...
public extension String {
    ...
    var hb_paramaters: [String: String] {
    ...
    var securePhoneStr: String {
        ...
        let headerStr = self.hb_subText(to: 2)
        var moddleStr = "****"
        var footerStr: String!
        if self.count > 7 {
            footerStr = self.hb_subText(from: 7, to: self.count - 1)
        } else {
            footerStr = ""
            moddleStr = ""
    ...
    /// - Returns: date
    func hb_toDate(formatter: String) -> Date {
    ...
    /// - Returns: 日期字符串
    static func hb_timeStampToDateString(_ timeStamp: String, output: String = "yyyy年MM月dd日 HH:mm:ss") -> String {
        let newTimeStamp = timeStamp.count > 10 ? (timeStamp.hb_subText(to: 9)): (timeStamp)
    ...
    func hb_compareToStr(_ otherTime: String, formatter: String) -> Int {
        let resultDic: [ComparisonResult: Int] = [.orderedSame: 0, .orderedAscending: 1, .orderedDescending: 2]
        let t1 = self.hb_toDate(formatter: formatter)
        let t2 = otherTime.hb_toDate(formatter: formatter)
    ...
    //MARK: caches路径
    /// caches路径
    static var hb_cachesPath: String {
    ...
    //MARK: documents路径
    /// documents路径
    static var hb_documentsPath: String {
...
public extension String {
    ...
    ///
    /// - Returns: 字符串
    func hb_removeHeadAndFoot() -> String {
    ...
    /// - Returns: 结果字符串
    @discardableResult
    mutating func hb_insert(_ text: String, at index: Int) -> String {
    ...
    /// - Returns: 结果字符串
    @discardableResult
    mutating func hb_insert(_ text: Character, at index: Int) -> String {
        if index > count - 1 || index < 0 {
            return self
        }
        let insertIndex = self.index(startIndex, offsetBy: index)
    ...
    /// - Returns: 结果字符串
    @discardableResult
    mutating func hb_remove(_ text: String) -> String {
    ...
    /// - Returns: 结果字符串
    @discardableResult
    mutating func hb_remove(at index: Int, length: Int) -> String {
    ...
    /// - Returns: 结果字符串
    @discardableResult
    mutating func hb_remove(at index: Int) -> String {
        if index > count - 1 || index < 0 {
            return self
        }
    ...
    ///   输出: str = "123456789"
    /// - Returns: 新字符串
    func hb_subText(from: Int = 0, to: Int) -> String {
    ...
    subscript(_ from: Int, _ to: Int) -> String {
        ...
        return self.hb_subText(from: from, to: to)
    }
    ...
    ///   - replaceStr: 替换的文字
    /// - Returns: 新字符串
    func hb_replaceStr(range: NSRange, replaceStr: String) -> String {
...
public extension String {
    ...
    ///
    /// - Returns: 是/否
    func hb_containsEmoij() -> Bool {
        
        return self.hb_isRegularCorrect("[^\\u0020-\\u007E\\u00A0-\\u00BE\\u2E80-\\uA4CF\\uF900-\\uFAFF\\uFE30-\\uFE4F\\uFF00-\\uFFEF\\u0080-\\u009F\\u2000-\\u201f\r\n]")
    }
    
    /// 移除字符串中的Emoij
    ///
    /// - Returns: 新字符串
    func hb_deleteEmoij() -> String {
        
        return self.hb_removeMatchRegular(expression: "[^\\u0020-\\u007E\\u00A0-\\u00BE\\u2E80-\\uA4CF\\uF900-\\uFAFF\\uFE30-\\uFE4F\\uFF00-\\uFFEF\\u0080-\\u009F\\u2000-\\u201f\r\n]", with: "")
    }
    ...
    /// 是否是数字
    var hb_isNumber: Bool {
        return self.hb_isRegularCorrect("^[0-9]+$")
    }
    
    /// 是否是字母
    var hb_isLetter: Bool {
        return self.hb_isRegularCorrect("^[A-Za-z]+$")
    }
    
    /// 是否符合邮箱规则
    var hb_isEmail: Bool {
        return self.hb_isRegularCorrect("^([A-Za-z0-9_\\-\\.\\u4e00-\\u9fa5])+\\@([A-Za-z0-9_\\-\\.\\u4e00-\\u9fa5])+\\.([A-Za-z\\u4e00-\\u9fa5]+)$")
    }
    
    /// 是否包含汉字
    var hb_isHasChinese: Bool {
        for chara in self {
    ...
    /// 是否符合手机号码规则
    var hb_isPhoneNum: Bool {
        
        // 全是数字, 不是空格
        return !self.hb_isEmpty && self.trimmingCharacters(in: CharacterSet.decimalDigits).count == 0 && !self.hb_isHasChinese
    }
    
    /// 密码是否符合规则 6-16位字母或数组
    var hb_isPassword: Bool {
        return self.hb_isRegularCorrect("^[^\\u4E00-\\u9FA5\\uF900-\\uFA2D\\u0020]{6,16}")
    }
    
    /// 是否符合身份证规则
    var hb_isIdCard: Bool {
        
        return self.hb_isRegularCorrect("^(\\d{14}|\\d{17})(\\d|[xX])$")
    }
    ...
    /// - Parameter str: str
    /// - Returns: 是否
    func hb_isRegularCorrect(_ str: String) -> Bool {
...
public extension String {
    ...
    /// - Returns: 结果
    func hb_isMatchRegular(expression: String) -> Bool {
    ...
    /// - Parameter expression: 正则表达式
    /// - Returns: 结果
    func hb_isContainRegular(expression: String) -> Bool {
        if let regularExpression = try? NSRegularExpression.init(pattern: expression, options: NSRegularExpression.Options.caseInsensitive) {
    ...
    ///   - newStr: 替换后的文字
    /// - Returns: 新字符串
    func hb_removeMatchRegular(expression: String, with newStr: String) -> String {
        if let regularExpression = try? NSRegularExpression.init(pattern: expression, options: NSRegularExpression.Options.caseInsensitive) {
    ...
    /// - Returns: [位置]?
    func hb_matchRegularRange(expression: String) -> [NSRange]? {
        if let regularExpression = try? NSRegularExpression.init(pattern: expression, options: NSRegularExpression.Options.caseInsensitive) {
...
public extension String {
    ...
    ///
    /// - Returns: 新字符串
    func hb_noWhiteSpaceString() -> String {
        var newStr = self
    ...
    ///
    /// - Returns: 新字符串
    func hb_removeEnterString() -> String {
        var newStr = self
        newStr = newStr.replacingOccurrences(of: "\n", with: "<br/>")
        newStr = newStr.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
...
public extension String {
    ...
    ///   - font: 字体
    /// - Returns: 宽度
    func hb_boundingWidth(height: CGFloat, font: UIFont) -> CGFloat {
        let label = UILabel()
    ...
    ///   - font: 字体大小
    /// - Returns: 尺寸
    func hb_boundingSize(size: CGSize, font: UIFont) -> CGSize {
...
public extension String {
    ...
    /// - Returns: NSRange
    func hb_toNSRange(from range: Range<String.Index>) -> NSRange? {
...
public extension String {
    ...
    ///   - textColor: 文字颜色
    /// - Returns: 图片
    func hb_toTextImage(font: UIFont, textColor: UIColor) -> UIImage? {
        
        let imgHeight: CGFloat = 16.0
        let imgWidth = self.hb_boundingSize(size: CGSize(width: UIScreen.main.bounds.width, height: imgHeight), font: font).width
...
extension String {
    ...
    ///   - centerImg: 中间的小图
    ///   - block: 回调
    public func hb_createQRCode(centerImg: UIImage? = nil) -> UIImage? {
...
public extension String {
    ...
    "{\"text\": \"zcc\"}".hb_convertToModel(modelType: TestModel.self)
    */
    /// Json转Model模型工具
    /// 数据模型的属性必须跟接口返回的数据类型相匹配!!!
    /// - Parameter modelType: T.Type
    /// - Returns: 数据模型
    func hb_convertToModel <T: Decodable>(modelType: T.Type) -> T? {
